...
dtest = pd.read_csv('./data/test.csv', na_values='NULL')
...
embedding_index = {}
for line in open('data/glove.6B.100d.txt'):
	line = line.strip()
	segs = line.split()
	word = segs[0]
...
for word, index in tokenizer.word_index.items():
	if index <= MAX_NB_WORDS and word in embedding_index:
		init_embed[index] = embbedding_index[word]
